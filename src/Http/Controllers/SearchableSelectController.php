...
namespace Pcode\SearchableSelect\Http\Controllers;
...
class SearchableSelectController extends Controller
{
    ...
    {
        $searchable = $request->get("searchable", true);
        $resourceClass =  $resource = $request->resource();
        ...
        if ($searchable && $request->filled('search')) {
            ...
            $query = app()->make(QueryBuilder::class, [$resourceClass])->search(
                $request, $resourceClass::newModel()->newQuery()->with($resourceClass::$with),
                $request->get('search')
            );

            $items = $query->limit($resourceClass::$globalSearchResults);
        ...
        $items = $items->get()->makeVisible(['display', 'value'])->each(function ($item) use ($request, $labelPrefix, $label) {
            $item->display = '';
            if($labelPrefix) {
                $item->display .= substr($item->{$labelPrefix}, 0, 50) ;
            }
